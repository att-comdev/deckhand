...
class TestRevisionTags(base.TestDbBase):
    ...
    def test_create_multiple_check_and_list(self):
        tags = []
        for _ in range(4):
            tag = test_utils.rand_name(self.__class__.__name__ + '-Tag')
            db_api.revision_tag_create(tag, self.revision_id)
            tags.append(tag)
        ...
        for tag in tags:
            ...
            db_api.revision_tag_check(tag, self.revision_id)
    ...
    def test_create_and_delete_tags(self):
        ...
        for idx, tag in enumerate(tags):
            ...
            result = db_api.revision_tag_delete(tag, self.revision_id)
    ...
    def test_replace_all_tags(self):
        ...
        new_tags = [test_utils.rand_name(self.__class__.__name__ + '-Tag')
                    for _ in range(4)]
        db_api.revision_tag_replace_all(new_tags, self.revision_id)
