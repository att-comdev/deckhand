...
class TestRevisionTags(base.TestDbBase):
    ...
    def test_create_show_and_list_many_tags_with_data(self):
        ...
        # Validate that each tag has the expected attributes.
        expected_attrs = ('created_at', 'updated_at', 'revision_id', 'tag',
                          'data')
        for tag in retrieved_tags:
            for attr in expected_attrs:
                self.assertIn(attr, tag)
                # Remove extra attributes so equivalence can be checked.
                if attr not in ('tag', 'data'):
                    tag.pop(attr)

        self.assertEqual(expected_tags, retrieved_tags)

        for idx, tag in enumerate(expected_tags):
            retrieved_tag = db_api.revision_tag_get(self.revision_id,
                                                    tag['tag'])

            # Validate that each tag has the expected attributes.
            for attr in expected_attrs:
                self.assertIn(attr, retrieved_tag)
                # Remove extra attributes so equivalence can be checked.
                if attr not in ('tag', 'data'):
                    retrieved_tag.pop(attr)

            self.assertEqual(expected_tags[idx], retrieved_tag)
