...
class TestRevisionDiffing(base.TestDbBase):
    ...
    def test_revision_diff_include_bucket_with_one_unrelated_document(self):
        payload = base.DocumentFixture.get_minimal_multi_fixture(count=3)
        ...
        # *Only* create 1 new document, with 2 unchanged documents from
        # previous revision.
        alt_payload[0]['name'] = test_utils.rand_name('foo')
        alt_payload[0]['schema'] = test_utils.rand_name('foo')
    ...
    def test_revision_diff_include_bucket_with_two_unrelated_documents(self):
        payload = base.DocumentFixture.get_minimal_multi_fixture(count=3)
        alt_payload = copy.deepcopy(payload)
        bucket_name = test_utils.rand_name('bucket')
        alt_bucket_name = test_utils.rand_name('bucket')
