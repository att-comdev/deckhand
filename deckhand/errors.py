...
class MissingDocumentKey(DeckhandException):
    ...
    **Message:** *Missing document key <key> from either parent or child. "Parent: <parent> Child: <child>*.
...
class ValidationNotFound(DeckhandException):
    ...
    **Message:** *The requested validation entry <entry_id> was not found for validation
    name <validation_name> and revision ID <revision_id>*.
...
class DocumentExists(DeckhandException):
    """
    A document attempted to be put into a bucket where another document with the same
    schema and metadata.name already exist.

    **Message:** *Document with schema <schema> and metadata.name <name> already exists in bucket <bucket>*.
...
class SubstitutionFailure(DeckhandException):
    ...
    **Message:** *An unknown exception occurred while trying to perform substitution. Details: <detail>*.
