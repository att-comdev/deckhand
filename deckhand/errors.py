...
class InvalidDocumentFormat(DeckhandException):
    ...
    **Message:** *The provided document failed schema validation. Details:
    <various>*.
...
class MissingDocumentKey(DeckhandException):
    ...
    **Message:** *Missing document key <key> from either parent or child.
...
class RevisionTagBadFormat(DeckhandException):
    ...
    **Message:** *The requested tag data <data> must either be null or
    dictionary*.
...
class RevisionTagNotFound(DeckhandException):
    ...
    **Message:** *The requested tag '<tag>' for revision <revision> was not
    found*.
...
class DocumentExists(DeckhandException):
    ...
    **Message:** *Document with schema <schema> and metadata.name <name>
...
class SingletonDocumentConflict(DeckhandException):
    ...
    **Message:** *A singleton document by the name <document> already exists
    in the system. The new document <conflict> cannot be created. To create a
    document with a new name, delete the current one first*.
...
class SubstitutionFailure(DeckhandException):
    ...
    **Message:** *An unknown exception occurred while trying to perform
    substitution. Details: <detail>*.
