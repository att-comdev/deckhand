...
class DocumentValidation(object):
    ...
    def _validate_one(self, document):
        ...
        doc_schema_type = self.SchemaType.get_schema(raw_dict)
        if doc_schema_type is None:
        ...
        try:
            ...
            # ignored.
            jsonschema.validate(raw_dict, doc_schema_type)
        except jsonschema.exceptions.ValidationError as e:
            if not document.is_abstract():
                error = errors.InvalidDocumentFormat(
                    detail=e.message, schema=e.schema,
                    document_type=document['schema'])
                debug_doc = self._gen_debug_doc(doc_schema_type, raw_dict)
                return error, debug_doc
            else:
    ...
    def _gen_debug_value(self, path):
        ...
        elif path in ['$.metadata', '$.data']:
            # Corce the data into a dict so it is compatible with DB col type.
