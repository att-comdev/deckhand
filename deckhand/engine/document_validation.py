...
class DocumentValidation(object):
    ...
    def __init__(self, documents):
        ...
        :param documents: Document(s) to be validated.
        :type documents: dict or List[dict]

        """

        self.documents = []
        db_data_schemas = db_api.revision_get_documents(
            schema=types.DATA_SCHEMA_SCHEMA, deleted=False)
        data_schemas = [
            document_wrapper.DocumentDict(d) for d in db_data_schemas
        ]
        _data_schema_map = {d['metadata']['name']: d for d in data_schemas}
        ...
        for document in documents:
            ...
            if document.schema.startswith(types.DATA_SCHEMA_SCHEMA):
                ...
                if document_name in _data_schema_map:
                    data_schemas.remove(_data_schema_map.pop(document_name))
