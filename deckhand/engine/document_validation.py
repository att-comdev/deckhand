...
class DataSchemaValidator(GenericValidator):
    ...
    def validate(self, document, pre_validate=True):
        ...
        for schema in schemas_to_use:
            ...
            # PRE-validate documents against built-in `DataSchema` documents,
            # in order to avoid raising spurious errors. These spurious errors
            # arise from `DataSchema` documents really only applying
            # post-rendering, when documents have all the substitutions they
            # need to pass externally registered `DataSchema` validations.
