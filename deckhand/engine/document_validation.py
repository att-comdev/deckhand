...
class BaseValidator(object):
    ...
    _schema_re = re.compile(r'^[a-zA-Z]+\/[a-zA-Z]+\/v[0-9](.0)?$')
...
class SchemaValidator(BaseValidator):
    ...
    def validate(self, document, validate_section=None,
        ...
        :param dict document: Document to validate.
        :param str validate_section: Document section to validate. If None,
        ...
        for schema_to_use in schemas_to_use:
            try:
                ...
                if validate_section:
                    jsonschema.validate(document.get(validate_section, None),
                                        schema_validator)
                else:
...
def is_abstract(document):
    try:
        is_abstract = document['metadata']['layeringDefinition']['abstract']
    except KeyError:
        return False
    return is_abstract
