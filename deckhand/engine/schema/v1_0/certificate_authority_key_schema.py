...
schema = {
    ...
    'properties': {
        'schema': {
            'type': 'string',
            'pattern': '^(deckhand/CertificateAuthorityKey/v[1]{1}(\.[0]{1}){0,1})$'
        },
        'metadata': {
            'type': 'object',
            'properties': {
...
"""JSON schema against which all documents with ``deckhand/CertificateAuthorityKey/v1``
``schema`` are validated.

.. literalinclude:: ../../deckhand/engine/schema/v1_0/certificate_authority_key_schema.py
   :language: python
   :lines: 15-49
