...
class DocumentLayering(object):
    ...
    def _calc_replacements_and_substitutions(
        ...
        def _check_replacement_and_parent_have_same_schema_and_name(
                parent, document):
        ...
        for document in self._documents_by_index.values():
            ...
            if document.is_replacement:
                _check_document_with_replacement_field_has_parent(
                    parent_meta, parent, document)
                _check_replacement_and_parent_have_same_schema_and_name(
                    parent, document)
            else:
    ...
    def _get_parent(self, doc, parent_meta):
        ...
        if parent.has_replacement:
            parent = (
                parent.replaced_by
                if parent.replaced_by is not doc else parent
            )
        return parent
    ...
    def render(self):
        ...
        for doc in self._sorted_documents:
            ...
            if doc.parent_selector:
                ...
                if parent_meta:
                    parent = self._get_parent(doc, parent_meta)
                    if parent.has_replacement:
                        parent = (
                            parent.replaced_by
                            if parent.replaced_by is not doc else parent
