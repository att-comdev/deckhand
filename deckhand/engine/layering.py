    ...
    def _validate_documents(self, documents):
        ...
        if val_errors:
            for error in val_errors:
                LOG.error('Document [%s] %s failed with validation error: %s.',
                          *error)
            raise errors.InvalidDocumentFormat(
                details='The following validation errors occurred '
                        '(schema, name, error): %s.' % val_errors)

    def __init__(self, documents, substitution_sources=None, validate=False):
        ...
        :param validate: Whether to validate documents using built-in schema
