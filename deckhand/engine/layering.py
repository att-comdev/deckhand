...
class DocumentLayering(object):
    ...
    def _apply_action(self, action, child_data, overall_data):
        ...
        # NOTE(fmontei): jsonpath_ng doesn't do "deep deleting" so we implement
        # it below.
        def _deep_delete(from_child, value, parent):
            if value == from_child:
                ...
                for v in value:
                    found = _deep_delete(from_child, v, value)
                    if found:
                        return True
        ...
        if method == 'delete':
            if action_path == '.':
                ...
                _deep_delete(from_child, overall_data.data, None)
