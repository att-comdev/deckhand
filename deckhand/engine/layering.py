...
class DocumentLayering(object):
    ...
    def _calc_document_replacements(self, substitution_sources):
        ...
        # TODO(fmontei): Move this somewhere where both secrets_manager and
        # this module only do this once. Right now this is repeated in
        # secrets_manager.
        sources = {}
        for src in substitution_sources:
            src = document_wrapper.DocumentDict(src)
            if src.meta in self._documents_by_index:
                document = self._documents_by_index[src.meta]
                if document.is_replacement:
                    # If the document replaces its parent, then simply
                    # overwrite the existing entry.
                    sources[(src.schema, src.name)] = src
            # If the replacer document is already populated, it won't be
            # replaced by the document getting replaced, due to `setdefault`
            # logic.
            sources.setdefault((src.schema, src.name), src)
    ...
    def _topologically_sort_documents(self, sources):
        ...
        for document in self._documents_by_index.values():
            ...
            for sub in document.substitutions:
                src = sources[(sub['src']['schema'], sub['src']['name'])]
    ...
    def __init__(self, documents, substitution_sources=None, validate=True,
        ...
        sources = self._calc_document_replacements(substitution_sources)

        self.secrets_substitution = secrets_manager.SecretsSubstitution(
            substitution_sources or [],
            fail_on_missing_sub_src=fail_on_missing_sub_src)

        self._sorted_documents = self._topologically_sort_documents(sources)
