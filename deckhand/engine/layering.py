...
class DocumentLayering(object):
        ...
        for document in self._documents_by_index.values():
            ...
            if document.is_replacement:
                ...
                if document.has_replacement:
                    error_message = (
                        'A replacement document cannot itself be replaced by '
                        'another document')
                    raise errors.InvalidDocumentReplacement(
                        schema=document.schema, name=document.name,
                        layer=document.layer, parent_schema=parent.schema,
                        parent_name=parent.name, parent_layer=parent.layer,
                        reason=error_message)

                if not parent_meta or not parent:
                    try:
                        parent_schema, parent_name, parent_layer = parent_meta
                    except Exception:
                        parent_schema, parent_name, parent_layer = (
                            'Unknown',) * 3
                    ...
                    raise errors.InvalidDocumentReplacement(
                        schema=document.schema, name=document.name,
                        layer=document.layer, parent_schema=parent_schema,
                        parent_name=parent_name, parent_layer=parent_layer,
                        reason=error_message)
    ...
    def _topologically_sort_documents(self):
    ...
    def __init__(self, documents, substitution_sources=None, validate=True,
        ...
        self._substitution_sources = self._calc_replacements_and_substitutions(
        ...
        del self._substitution_sources
