...
class SecretsManager(object):
    ...
    def create(cls, secret_doc):
        ...
        if encryption_type == types.ENCRYPTED:
            # NOTE(fmontei): The logic for the 2 conditions below is enforced
            ...
            if not payload:
                ...
                try:
                    ...
                    LOG.error('Failed to base64-encode payload of type %s '
                              'for Barbican storage.', type(payload))
                    LOG.exception(e)
                raise errors.UnknownSubstitutionError(
                    src_schema=secret_doc.schema, src_layer=secret_doc.layer,
                    src_name=secret_doc.name, schema='N/A', layer='N/A',
                    name='N/A', details=six.text_type(e))
