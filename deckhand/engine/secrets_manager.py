...
class SecretsManager(object):
    ...
    def get(cls, secret_ref):
        ...
        # NOTE(fmontei): The only way to distinguish between a Barbican secret
        # reference and everything else is to engage in matching footwork
        # because this entire module needs to work in offline mode, making a
        # Keystone endpoint look up impossible. The next best thing is to
        # check whether the reference is a URL and that it contains the
        # substring 'secrets' and that it ends in /{uuid}. If someone believes
        # that imitating Barbican references is a good idea, then all hope is
        # lost.
