...
def documents_create(bucket_name, documents, session=None):
    ...
    :bucket_name: The name of the bucket with which to associate created
        documents.
    ...
    if documents_to_delete:
        LOG.debug('Creating documents: %s.', documents_to_delete)
        deleted_documents = []
    ...
    for values in values_list:
        ...
        try:
            existing_document = document_get(
                raw_dict=True, bucket_id=bucket_name,
...
def revision_get_documents(revision_id=None, bucket_name=None,
                           include_history=True, unique_only=True,
                           session=None, **filters):
    ...
    :bucket_name: The name of the bucket to which the revision documents
        belong. Applied to ``filters`` in order to filter the retrieved docs
        by the desired bucket.
    ...
    if bucket_name:
        filters.update({'bucket_id': bucket_name})
