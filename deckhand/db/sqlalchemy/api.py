...
def _apply_filters(dct, **filters):
    ...
    for filter_key, filter_val in filters.items():
        ...
        if isinstance(filter_val, (list, tuple)):
            ...
            # if actual_val[0] not in filter_val:
            #     return False
...
@require_revision_exists
def validations_get_all(revision_id, session=None):
    session = session or get_session()
...
def validation_get_all_entries(revision_id, val_name, session=None):
    ...
    validation_policy = document_get(**filters)
    validations = validation_policy['data']['validations']
    validation_map = {}
    for v in validations:
        validation_map.setdefault(v['name'], [])
        validation_map[v['name']].append(v)
    return validation_map.get(val_name, [])
...
def validation_get_entry(revision_id, val_name, entry_id, session=None):
    ...
    target_validation = validation_map.get(val_name, [])
    try:
        return target_validation[entry_id]
    except IndexError:
        return {}
