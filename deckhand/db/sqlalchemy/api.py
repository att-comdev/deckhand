        ...
        def wrapper(bucket_name, documents, *args, **kwargs):
            ...
            if existing_document_names and conflicting_names:
                raise errors.SingletonDocumentConflict(
                    ...
                    name=existing_document_names[0][1],
                    layer=existing_document_names[0][2],
                    conflict=', '.join(["[%s, %s] %s" % (x[0], x[2], x[1])
...
def documents_create(bucket_name, documents, validations=None,
    ...
    document_history = [
        d.meta for d in revision_documents_get(bucket_name=bucket_name)
    ]
    documents_to_delete = [
        h for h in document_history if h not in [d.meta for d in documents]
    ]
    ...
    for document in documents:
        ...
        if existing_document:
            ...
            if (existing_document['data_hash'] == document['data_hash'] and
                existing_document['metadata_hash'] == document['metadata_hash']
                and existing_document['layer'] == document['layer']):
