...
def require_unique_layering_policy(f):
    ...
    :raises ConflictingLayeringPolicy if a layering policy in the system already
        exists and any of the documents to be created is also a layeringPolicy
        but has a name that differs from the one already registered.
    """
    @functools.wraps(f)
