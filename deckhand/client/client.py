...
data: |-
  ...
  -----END CERTIFICATE-----
"""
    result = client.buckets.update('foo', test)
    print(result)

    result = client.revisions.diff(1, 1)
    print(result.to_dict())

    result = client.revisions.get(1)
    print(result.to_dict())

    result = client.revisions.list()
    print(result.to_dict())

    result = client.revisions.documents(1, rendered=True)
    print([r.to_dict() for r in result])

    result = client.revisions.documents(1, rendered=False)
    print([r.to_dict() for r in result])

    result = client.tags.create(1, 'bar', data={'a': 'b', 'c': 'd'})
    print(result.to_dict())

    result = client.tags.list(1)
    print([r.to_dict() for r in result])

    result = client.tags.get(1, 'bar')
    print(result.to_dict())

    result = client.tags.delete(1, 'bar')
    print(result is None)

    result = client.tags.delete_all(1)
    print(result is None)
...
validator:
  ...
  version: 1.1.2
"""
    result = client.validations.create(1, 'foo_validation', validation)
    print(result.to_dict())

    result = client.validations.list(1)
    print([r.to_dict() for r in result])

    result = client.validations.list_entries(1, 'deckhand-schema-validation')
    print(result.to_dict())

    result = client.validations.get_entry(1, 'deckhand-schema-validation', 0)
    print(result.to_dict())
