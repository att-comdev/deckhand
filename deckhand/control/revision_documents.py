...
class RenderedDocumentsResource(api_base.BaseResource):
    ...
    def on_get(self, req, resp, sanitized_params, revision_id):
        ...
        # TODO(fmontei): Only apply internal filters pre-rendering, then
        # apply user-provided filters post-rendering, because many documents
        # that might not be contained in the final query are needed for
        # rendering.
        ...
        try:
            ...
            error_msg = (
                'No layering policy found for revision %s, so could not '
                'render the documents.' % revision_id)
            LOG.error(error_msg)
            LOG.exception(six.text_type(e))
